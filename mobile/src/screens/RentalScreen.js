...
const RentalScreen = ({ navigation }) => {
    ...
    const { data: rentalFleet = [], isLoading: isLoadingData, mutate } = useSWR(
        RENTAL_URL,
        fetcher,
    ...
    useEffect(() => {
        ...
        socket.on('fleet_updated', () => mutate());
...
const getStyles = (COLORS) => StyleSheet.create({
    ...
    // Requirements
    reqCard: { backgroundColor: COLORS.cardBackground, borderRadius: 16, padding: 20, borderWidth: 1, borderColor: COLORS.border, marginBottom: 16 },
    reqTitle: { fontSize: 16, fontWeight: '800', color: COLORS.primary, marginBottom: 12 },
    reqSubtitle: { fontSize: 13, color: COLORS.textMuted, marginBottom: 16, lineHeight: 20 },
    reqItem: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12, gap: 10 },
    reqCheck: { color: '#22c55e', fontSize: 16, fontWeight: '800' },
    reqText: { flex: 1, fontSize: 13, color: COLORS.text, fontWeight: '500', lineHeight: 20 },
