...
const TopHeader = ({ employee, title, subtitle, onNavigate }) => {
    ...
    const markAllRead = async () => {
        try {
            await axios.patch(`${API_BASE}/notifications/mark-read`, {}, { headers: authHeaders() });
    ...
    const deleteAll = async () => {
        try {
            await axios.delete(`${API_BASE}/notifications/delete-all`, { headers: authHeaders() });
            setNotifications([]);
            setUnreadCount(0);
            setIsOpen(false);
        } catch (err) { console.error(err); }
    ...
    const handleNotifClick = async (notif) => {
        try {
            if (!notif.isRead) {
                await axios.patch(`${API_BASE}/notifications/${notif._id}/read`, {}, { headers: authHeaders() });
...
const EmployeeDashboard = () => {
    ...
    const renderContent = () => {
        switch (toggleActive) {
            ...
            case 'bookings':
                return <BookingManagement employee={employee} onNavigate={setToggleActive} />;
            case 'car-rent':
                return <CarRentManagement employee={employee} onNavigate={setToggleActive} />;
            default:
                return <DashboardOverview employee={employee} onNavigate={setToggleActive} />;
        }
    };
...
const DashboardOverview = ({ employee, onNavigate }) => {
    ...
    return (
        <div>
            {/* Header */}
            <TopHeader employee={employee} title="System Overview" subtitle={todayDate} onNavigate={onNavigate} />
...
const BookingManagement = ({ employee, onNavigate }) => {
    ...
    useEffect(() => {
        ...
        socket.on('new_booking', (booking) => {
            ...
            showToast(`New booking arrived: ${booking.firstName} ${booking.lastName}`);
        });
    ...
    return (
        <div className="position-relative">
            ...
            <TopHeader employee={employee} title="Manage Bookings" subtitle="View and update all carwash bookings" onNavigate={onNavigate} />
                                        ...
                                        <td>
                                            <select
                                                className={`form-select form-select-sm fw-medium shadow-none ${booking.status === 'Completed' ? 'border-success text-success' :
                                                    booking.status === 'Queued' ? 'border-primary brand-primary' :
...
/* ─────────────────────────────────────────────
   CAR RENT MANAGEMENT
───────────────────────────────────────────── */
const CarRentManagement = ({ employee, onNavigate }) => (
    <div>
        <TopHeader employee={employee} title="Manage Car Rentals" subtitle="Track and manage all active car rentals" onNavigate={onNavigate} />
